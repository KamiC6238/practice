...
'''
   ...
   已知一棵树是完全二叉树，求这个数的节点个数，要求时间复杂度不能为O(n):如果是O(N),直接层序遍历就能搞定了。现在不能这样子了。所以
                                                                  还是递归。先判断根节点右子树的左边界的高度是否跟整棵树的高度
                                                                  一致，如果是，那么整棵树的左子树必为满二叉树，根据公式即可算出
                                                                  左子树的高度，接着以右子树的节点为根节点进行递归。如果不一致，
                                                                  则右子树是满二叉树，此时应该进入左子树进行递归计算节点个数。
   这个解法的时间复杂度，因为每一层都只选择一个节点，这是根据节点的右子树的左边界的高度来判断的，如果左边界不等于总高度，那么就选另一个
   节点。因此若有N个节点，那么高度为logN,在遍历左边界的这个过程，也是每次只经历左子树的节点而已，所以也是logN,所以总的时间复杂度就是
   O((logN)^2)
   
   感悟:这里面有几个题其实我都自己学习完想了很久，我发现我总是纠结于要搞清楚每一层递归到底都干了什么，我越这样做，我就越糊涂。
        但其实没必要这样，因为递归的每一步都是高度一致的，因此我们只要想好其中一步，就能够代替全部的过程，因此我实际上是没必要
        去纠结这个问题的。
'''
